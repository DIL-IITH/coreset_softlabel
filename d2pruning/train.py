...
for exp in range(args.experiments):
    ...
    while num_of_iterations > 0:
        ...
        trainer.train(current_epoch, -1, model, trainloader, optimizer, criterion, scheduler, device, TD_logger=TD_logger, log_interval=60, printlog=False)
...
file=open('{}_result_hl.txt'.format(args.dataset),'a+')
text_to_write="{},{:.3f},{:.3f}".format(coreset_ratio,np.mean(list_of_best_acc),np.std(list_of_best_acc))
