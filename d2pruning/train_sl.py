...
for exp in range(args.experiments):
    ...
    teacher_model = resnet('resnet18', num_classes=num_classes, device=device)
    checkpoint = torch.load(args.arch_path,map_location=device)
    ...
    while num_of_iterations > 0:
        ...
        trainer.train(current_epoch, -1, model, trainloader, optimizer, criterion, scheduler, device, TD_logger=TD_logger, log_interval=60, printlog=False,use_soft_label=True,teacher_model=teacher_model,temperature=args.temperature,criterion_kl=criterion_kl)

        num_of_iterations -= iterations_per_epoch
        if current_epoch>450 and (current_epoch % epoch_per_testing == 0 or num_of_iterations == 0):
            test_loss, test_acc = trainer.test(model, testloader, criterion, device, log_interval=20,  printlog=False)
        ...
        pbar.set_postfix_str("Best acc: {:.3f}, best epoch:{}, itr: {}".format(best_acc*100.0,best_epoch,exp+1))
        # scheduler.step()
...
file=open('{}_result_sl.txt'.format(args.dataset),'a+')
text_to_write="{},{},{:.3f},{:.3f}".format(coreset_ratio,args.temperature,np.mean(list_of_best_acc),np.std(list_of_best_acc))
