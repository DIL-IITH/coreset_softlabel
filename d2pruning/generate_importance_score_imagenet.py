...
from core.data import IndexDataset, CIFARDataset, SVHNDataset, CINIC10Dataset, TinyImageNetDataset,ImageNetDataset
from core.utils import print_training_info, StdRedirect

class ResNet34_Features(nn.Module):
    def __init__(self, pretrained=False,arch_path=None):
        super().__init__()
        self.model = models.resnet34(pretrained=pretrained)
        if arch_path:
            print(f'Ckpt path: {arch_path}.')
            checkpoint = torch.load(arch_path)['model_state_dict']
    ...
    def feature_map(self, x, layer=4):
        # Stem
        ...
        raise ValueError("layer argument must be 3 or 4")

    def forward(self, x):
        # normal classifier forward
        out = self.feature_map(x, layer=4)
        return self.fc(out)
...
parser.add_argument('--dataset', type=str, default='cifar10', choices=['cifar10', 'cifar100', 'tiny', 'svhn', 'cinic10', 'tinyimagenet','imagenet'])
...
if dataset == 'cifar10':
    ...
    trainset = ImageNetDataset.get_ImageNet_train(os.path.join(args.data_dir, 'train'),transform=transform_identical)
...
model = ResNet34_Features(pretrained=False,arch_path=ckpt_path)
