...
def main():
    ...
    parser.add_argument("--eval_start_epoch",type=int,default=150)
    ...
    for exp in range(start_exp, args.num_exp):
        ...
        for model in models:
            ...
            teacher_network = nets.__dict__[args.model](channel, num_classes, im_size).to(args.device)
            ...
            print("Teacher network: ",args.model)
            print("Student network: ",model)
            network = nets.__dict__[model](channel, num_classes, im_size).to(args.device)

            if args.device == "cpu":
                print("Using CPU.")   
            network=network.to(args.device)
            print("[INFO] using {}".format(args.device))

            if "state_dict" in checkpoint.keys():
                # Loading model state_dict
                network.load_state_dict(checkpoint["state_dict"])

            criterion_hard = nn.CrossEntropyLoss(reduction='none').to(args.device)
            ...
            for epoch in pbar:
                # train for one epoch
                train_soft_label(train_loader, network,teacher_network, args.temperature,criterion, optimizer, scheduler, epoch, args, rec, if_weighted=if_weighted,device=args.device)

                # evaluate on validation set
                if epoch > args.eval_start_epoch and args.test_interval > 0 and (epoch + 1) % args.test_interval == 0:
                    prec1 = test(test_loader, network, criterion_hard, epoch, args, rec)
                ...
                pbar.set_postfix_str("best test accuracy:{:.3f}".format(best_prec1))
...
if __name__ == '__main__':
    ...
    print(np.std(best_acc_list))
    end_time = time.time()
    print("[INFO] Total average time taken : {:.3f} seconds".format((end_time-start_time)/args.num_exp)) 
    file = open("results_soft_labels.txt","a+")
    results = "Method: {}, Fraction : {}, mean: {:.3f}, std: {:.3f}\n".format(args.selection,args.fraction,np.mean(best_acc_list),np.std(best_acc_list))
